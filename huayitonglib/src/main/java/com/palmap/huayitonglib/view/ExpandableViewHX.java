...
public class ExpandableViewHX extends LinearLayout implements View.OnClickListener {
    ...
    public ExpandableViewHX(Context context) {
        this(context, null);
    }

    public ExpandableViewHX(Context context, AttributeSet attrs) {
        super(context, attrs);
        init(context, attrs);
    }

    @TargetApi(Build.VERSION_CODES.HONEYCOMB)
    public ExpandableViewHX(Context context, AttributeSet attrs, int defStyle) {
...
//可以自行调节字体的大小,sp转px貌似很大啊,所以使用px转dp
        title_size = typedArray.getDimension(R.styleable.ExpandableView_viewTitleSize,
                50);
        title_size= 14; //DisplayUtil.px2dip(mContext, 14);
        title_color = typedArray.getColor(R.styleable.ExpandableView_viewTitleColor, getResources().getColor(R.color.text_333));
        line_color = typedArray.getColor(R.styleable.ExpandableView_viewTitleLineColor, getResources().getColor(R.color.white));
        if (mTitlt != null) {
            initTitleText();
        }
        typedArray.recycle();
    ...
    private void initTitleText() {
        int padLeft = DisplayUtil.dip2px(mContext, 23);
        int h = 0;
        if (mTitleImage != null) {
